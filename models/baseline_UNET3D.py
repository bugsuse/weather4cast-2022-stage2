...
class DownConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dropout_rate, pooling=True, planar=False, activation='relu',
                 normalization=None, full_norm=True, dim=3, conv_mode='same', bias=True):
        super().__init__()
        ...
        self.conv1 = conv3(
            self.in_channels, self.out_channels, planar=planar, dim=dim, padding=padding, bias=bias
        )
        self.conv2 = conv3(
            self.out_channels, self.out_channels, planar=planar, dim=dim, padding=padding, bias=bias
        )
...
class UpConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
                 ...
                 activation='relu', normalization=None, full_norm=True, dim=3, sub_sample_factor=2,
                 conv_mode='same', attention=False, bias=True):
        super().__init__()
        ...
        if self.merge_mode == 'concat':
            self.conv1 = conv3(
                2*self.out_channels, self.out_channels, planar=planar, dim=dim, padding=padding, bias=bias
            )
        else:
        ...
        if 'grid' in attention:
            self.attention = GridAttention(
                in_channels=in_channels // 2, gating_channels=in_channels, dim=dim, sub_sample_factor=(1, 2, 2)
            )
        elif 'selayer' in attention:
            self.attention = SELayer(out_channels, 16)
        else:
